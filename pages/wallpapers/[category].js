...
export default function WallpaperCategoryPage({ category, products }) {
  if (!category) return null;

  return (
    <Layout title={`${category.name} Wallpapers — Luminance`}>
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <nav className="flex items-center gap-2 mb-10 text-xs font-body text-ash-500">
          ...
          <span>/</span>
          <Link href="/wallpapers" className="hover:text-ash-300 transition-colors">Wallpapers</Link>
          <span>/</span>
          <span className="text-ash-300">{category.name}</span>
        </nav>

        <div className="mb-12">
          <span className="badge badge-gold mb-3">Wallpaper</span>
...
export async function getStaticPaths() {
  const { data } = await supabaseAdmin.from('categories').select('slug').eq('type', 'wallpaper');
  return {
